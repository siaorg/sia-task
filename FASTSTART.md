...
最近微服务任务调度平台SIA-TASK开源，SIA-TASK属于分布式的任务调度平台，使用起来简单方便，非常容易入手，部署搭建好SIA-TASK任务调度平台之后，编写TASK进行调度，进而实现整个调度流程，本文新建了一个示例，该示例为一个JOB，该JOB关联了前后级联的TASK，TASKOne(前置TASK)和TASKTwo(后置TASK)，本示例来阐述一个TASK(执行器)是如何通过SIA-TASK实现任务调度的。
